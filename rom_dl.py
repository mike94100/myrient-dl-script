...
def is_collection_toml(file_path: Path) -> bool:
    """Check if TOML is a collection config (contains platforms section)"""
    try:
        config = parse_toml_file(str(file_path))
        return 'platforms' in config
    except:
        return False


def get_collection_url_directory(toml_path: Path) -> Path:
    """Get the URL directory path for a collection TOML"""
    return toml_path.parent / "urls"
...
Examples:
  ...
  python rom_dl.py collections/sample/sample.toml --dry-run
    ...
    if is_collection_toml(input_path):
        # Process collection TOML - automatically find and use URL directory
        url_dir = get_collection_url_directory(input_path)
        logger.info(f"Detected collection TOML, using URL directory: {url_dir}")
        success = process_url_directory(url_dir, output_dir, args.dry_run, args.platforms)
